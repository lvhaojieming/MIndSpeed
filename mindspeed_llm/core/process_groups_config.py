...
class ProcessGroupCollection:
    ...
    def use_mpu_process_groups(cls, required_pgs: Optional[List[str]] = None):
        ...
        pg_to_func = {
            ...
            'embd': partial(parallel_state.get_embedding_group),
            'pos_embd': partial(
        ...
        if not all(pg in pg_to_func for pg in required_pgs):
            raise ValueError(f"Initialization function for process group not defined for all ProcessGroupCollection fields")

        # Build initialization dict by calling appropriate parallel_state get_foo_group
        init_dict = {pg: func() for pg in required_pgs if (func := pg_to_func.get(pg)) is not None}
