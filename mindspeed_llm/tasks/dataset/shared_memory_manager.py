...
class SharedMemoryManager:
    def __init__(self, base_shm_name, rank0_pid, buffer_length, tp_size,
        ...
        self.tensor = torch.frombuffer(self.shm.buf[offset:offset + buffer_length * self.int64_size],
                                       dtype=torch.int64).view((buffer_length,))
        offset += buffer_length * self.int64_size

        self.seq_len_real_length = torch.frombuffer(self.shm.buf[offset:offset + self.int64_size], dtype=torch.int64)
        offset += self.int64_size

        self.seq_len_num = torch.frombuffer(self.shm.buf[offset:offset + self.int64_size], dtype=torch.int64)
        offset += self.int64_size

        self.read_flags = torch.frombuffer(self.shm.buf[offset:offset + tp_size * self.int64_size], dtype=torch.int64)
        offset += tp_size * self.int64_size

        self.data_version = torch.frombuffer(self.shm.buf[offset:], dtype=torch.int64)

        if not existing:
            self.read_flags.zero_()
            self.data_version.zero_()
            self.seq_len_real_length.zero_()
            self.seq_len_num.zero_()
    ...
    def write(self, data):
        ...
        if data is None or real_length == 0:
            ...
            self.seq_len_real_length.fill_(-1)
            self.seq_len_num.fill_(-1)
        else:
            self.tensor[:real_length].copy_(data.view(-1)[:real_length])
            self.tensor[real_length:].fill_(0)
            self.seq_len_real_length.fill_(real_length)
            self.seq_len_num.fill_(seq_len_num)

        self.data_version.add_(1)
        self.read_flags[self.rank] = 1
    ...
    def read(self):
        ...
        if real_length == -1:
            ...
            if seq_len_num <= 1:
                data = self.tensor[:real_length].clone()
            else:
                data = self.tensor[:real_length].clone().view(seq_len_num, -1)
