...
class MiniMaxM2SparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        if args.moe_grouped_gemm:
            self.experts = MiniMaxM2FusedExperts(config)
        else:
    ...
    def route_tokens_to_experts(self, router_logits):
        ...
        return top_k_index, top_k_weights.to(router_logits.dtype), routing_weights
    ...
    def forward(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        top_k_index, top_k_weights, routing_weights = self.route_tokens_to_experts(router_logits)

        args = get_args()
        if args.fix_router:
            # Round-robin assignment for balanced expert load. Performance tuning only.
            num_tokens = hidden_states.shape[0]
            top_k_index = (
                torch.arange(num_tokens * self.top_k, device=hidden_states.device, dtype=torch.int64)
                .view(num_tokens, self.top_k)
                .remainder(router_logits.shape[-1])
            )
            top_k_weights = routing_weights.gather(1, top_k_index).to(router_logits.dtype)
            top_k_weights /= top_k_weights.sum(dim=-1, keepdim=True)
...
class MiniMaxM2FusedExperts(nn.Module):
    def __init__(self, config: MiniMaxM2Config):
        super().__init__()
        self.num_experts = config.num_local_experts
        self.hidden_dim = config.hidden_size
    ...
    def forward(self, hidden_states, top_k_index=None, top_k_weights=None):
        ...
        return npu_moe_token_unpermute(fc2_out, row_ids_map, probs=top_k_weights)

    def ep_forward(self, hidden_states: torch.Tensor, split_list: torch.Tensor) -> torch.Tensor:
        """
        Local expert computation under EP. Called after AllToAll dispatch,
        with hidden_states already arranged by expert. Routing weight scaling
        is handled by the framework's alltoall_combine, not here.
        ...
        # Reshape to (local_E, in, out) as required by GMM.
        gate_up_view = gate_up_weights.view(num_local_experts, self.hidden_dim, 2 * self.ffn_dim)
        down_view = down_weights.view(num_local_experts, self.ffn_dim, self.hidden_dim)
        ...
        if not isinstance(split_list, torch.Tensor):
            ...
            split_list = split_list.to(torch.int64)

        # w1+w3 → swiglu → w2
        fc1_output = Ops.gmm(hidden_states, gate_up_view, split_list, trans_b=False)
        fc1_activation = torch_npu.npu_swiglu(fc1_output, dim=-1)
        return Ops.gmm(fc1_activation, down_view, split_list, trans_b=False)
